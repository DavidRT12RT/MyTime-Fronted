    ...
    useEffect(() => {
        if(publicacion != null){
            ...
            publicacion.reacciones.forEach(reaccion => {
                if(reaccion.autor.uid == auth.uid) bandera = true;
            });
        ...
        <div className="publicacionContainer">
            ...
            <div className="userInformationContainer">
                <div className="content">
                    <Avatar size={40} src={`${process.env.REACT_APP_BACKEND_API_URL}/api/usuarios/${publicacion.autor.uid}/fotos/?tipo=perfil`} />
                    <div className="d-flex align-items-start flex-column">
                        <Link to={`/users/${publicacion.autor.uid}/`} target="_blank"><h1 className="titulo-descripcion" style={{margin:"0px"}}>{publicacion.autor.nombre}</h1></Link>
                        <p style={{margin:"0px"}}>{moment(publicacion.fecha,"LLLL").fromNow()}</p>
                    </div>
                </div>

                {
                    publicacion.autor.uid === auth.uid &&
                    ...
                    <div className="fotosContainer">
                        {
                            publicacion.fotos.map((foto) => {
                                return <img src={`${process.env.REACT_APP_BACKEND_API_URL}/api/publicaciones/${publicacion._id}/fotos/${foto}`} key={foto}/>
                            })
                        }
                    </div>
            }
