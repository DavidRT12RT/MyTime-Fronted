...
const UsuarioNewPublication = ({userInfo}) => {
    ...
    if(userInfo === null) return <h1>Cargando informacion usuario...</h1>
    else return (            
        <>
            <div className="newPublicUserContainer" id="form" onSubmit={handleAddNewPublic}>
                <Avatar size={40} src={`${process.env.REACT_APP_BACKEND_API_URL}/api/usuarios/${userInfo.uid}/fotos/?tipo=perfil`} />
                    ...
                    isModalVisible.tipoPublicacion && (
                        <>
                            ...
                            <div className="containerTiposPublicacion">
                                <button type="primary" className={"btnTypeOfPublic descripcion " + (values.tipoPublicacion == "normal" && "primary-bg-color")} onClick={() => setValues((values) => ({...values,tipoPublicacion:"normal"}))}><span className="me-2"><BookmarkPlusFill/></span>Normal</button>
                                <button type="primary" className={"btnTypeOfPublic descripcion " + (values.tipoPublicacion == "anuncio" && "primary-bg-color")} onClick={() => setValues((values) => ({...values,tipoPublicacion:"anuncio"}))}><span className="me-2"><MegaphoneFill/></span>Anuncio</button>
                                <button type="primary" className={"btnTypeOfPublic descripcion " + (values.tipoPublicacion == "noticias" && "primary-bg-color")} onClick={() => setValues((values) => ({...values,tipoPublicacion:"noticias"}))}><span className="me-2"><Newspaper/></span>Noticia</button>
                            </div>
                        </>
                    )
                }
                <div className="row">
                    <button type="primary" className="btn btn-primary mt-3" onClick={handleAddNewPublic}>Publicar</button>
                </div>
            </Modal>
        </>
    )
